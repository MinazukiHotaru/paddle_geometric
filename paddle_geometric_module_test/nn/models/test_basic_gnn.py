...
from paddle_geometric.nn import SAGEConv
from paddle_geometric.nn.models import GAT, GCN, GIN, PNA, EdgeCNN, GraphSAGE
...
out_dims = [None, 8]
dropouts = [0.0, 0.5]
acts = [None, 'leaky_relu', paddle.relu_, F.elu, paddle.nn.ReLU()]
...
def test_gcn(out_dim, dropout, act, norm, jk):
    ...
    assert model(x, edge_index).shape== (3, out_channels)
...
def test_jit():
    ...
    assert model(x, edge_index).shape== (3, 16)
...
def test_basic_gnn_inference(get_dataset, jk):
    ...
    assert out1.shape== (data.num_nodes, dataset.num_classes)

    loader = NeighborLoader(data, num_neighbors=[-1], batch_size=128)
    out2 = model.inference(loader)
    assert out1.shape== out2.size()
    assert paddle.allclose(out1, out2, atol=1e-4)
...
def test_compile_basic(device):
    x = paddle.randn(shape=[3, 8, place=device])
...
def test_packaging():
    ...
    os.makedirs(paddle.hub._get_paddle_home(), exist_ok=True)

    x = paddle.randn(shape=[3, 8])
    edge_index = paddle.to_tensor([[0, 1, 1, 2], [1, 0, 2, 1]])

    model = GraphSAGE(8, 16, num_layers=3)
    path = osp.join(paddle.hub._get_paddle_home(), 'pyg_test_model.pdparams')
    ...
    model = GraphSAGE(8, 16, num_layers=3)
    path = osp.join(paddle.hub._get_paddle_home(), 'pyg_test_package.pdparams')
    with paddle.package.PackageExporter(path) as pe:
        pe.extern('paddle_geometric.nn.**')
        pe.extern('paddle_geometric.inspector')
        pe.extern('paddle_geometric.utils._trim_to_layer')
        pe.extern('_operator')
        pe.save_pickle('models', 'model.pkl', model)

    pi = paddle.package.PackageImporter(path)
...
def test_onnx(tmp_path):
    ...
    model = MyModel()
    x = paddle.randn(shape=[3, 8])
    edge_index = paddle.to_tensor([[0, 1, 2], [1, 0, 2]])
    expected = model(x, edge_index)
    assert expected.shape== (3, 16)

    path = osp.join(tmp_path, 'model.onnx')
    paddle.onnx.export(model, (x, edge_index), path,
...
def test_compile_graph_breaks(Model, device):
    ...
    if Model in {PNA}:  # `PNA` requires additional arguments:
        ...
        kwargs['deg'] = paddle.to_tensor([1, 2, 1])
...
if __name__ == '__main__':
    ...
    def gen_args():
        ...
        x = paddle.randn(shape=[N, 64, place=args.device])
        edge_index = paddle.randint(N, (2, E), place=args.device)
