...
def test_dot_syntax_keys():
    parameter_dict = {
        'param1': paddle.nn.Parameter(paddle.randn(shape=[16, 16])),
        'model.param2': paddle.nn.Parameter(paddle.randn(shape=[8, 8])),
        'model.sub_model.param3': paddle.nn.Parameter(paddle.randn(shape=[4, 4])),
    }
...
def test_tuple_keys():
    parameter_dict = {
        ('a', 'b'): paddle.nn.Parameter(paddle.randn(shape=[16, 16])),
        ('a.b', 'c'): paddle.nn.Parameter(paddle.randn(shape=[8, 8])),
    }
    parameter_dict = ParameterDict(parameter_dict)
...
def test_reserved_keys():
    parameter_dict = {
        'type': paddle.nn.Parameter(paddle.randn(shape=[16, 16])),
        '__annotations__': paddle.nn.Parameter(paddle.randn(shape=[8, 8])),
    }
    parameter_dict = ParameterDict(parameter_dict)
