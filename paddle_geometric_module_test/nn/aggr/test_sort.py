...
from paddle_geometric.nn.aggr import SortAggregation


def test_sort_aggregation():
    N_1, N_2 = 4, 6
    x = paddle.randn(shape=[N_1 + N_2, 4])
    index = paddle.to_tensor([0 for _ in range(N_1)] + [1 for _ in range(N_2)])
    ...
    assert paddle.allclose(out_dim, out)
    ...
    # Nodes are sorted.
    assert paddle.equal(out[0, :4, -1].argsort(), 3 - paddle.arange(4))
    assert paddle.equal(out[1, :, -1].argsort(), 4 - paddle.arange(5))
...
def test_sort_aggregation_smaller_than_k():
    ...
    # Nodes are sorted.
    assert paddle.equal(out[0, :4, -1].argsort(), 3 - paddle.arange(4))
    assert paddle.equal(out[1, :6, -1].argsort(), 5 - paddle.arange(6))
