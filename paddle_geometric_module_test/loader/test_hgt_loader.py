...
from paddle_geometric.loader import HGTLoader
...
def is_subset(subedge_index, edge_index, src_idx, dst_idx):
    ...
    mask = paddle.from_numpy(np.isin(subidx, idx))
...
def test_hgt_loader():
    ...
    full_adj = SparseTensor(
        row=paddle.concat([r1, r2, r3]),
        col=paddle.concat([c1, c2, c3]),
        value=paddle.arange(2500),
    )
    ...
    for batch in loader:
        ...
        assert batch['paper'].x.shape== (40, )  # 20 + 4 * 5
        ...
        assert batch['author'].x.shape== (20, )  # 4 * 5
        ...
        assert batch['paper', 'paper'].e_id.shape== (num_edges, )
        ...
        assert adj.nnz() == row.shape[0]
        assert paddle.allclose(row.unique(), adj.storage.row().unique())
        assert paddle.allclose(col.unique(), adj.storage.col().unique())
        assert paddle.allclose(value.unique(), adj.storage.value().unique())
        ...
        assert batch['paper', 'author'].e_id.shape== (num_edges, )
        ...
        assert batch['author', 'paper'].e_id.shape== (num_edges, )
        ...
        n_id = paddle.concat([batch['paper'].x, batch['author'].x])
        row, col, _ = full_adj[n_id, n_id].coo()
        assert paddle.concat([row, col]).unique().numel() >= 59
...
def test_hgt_loader_on_cora(get_dataset):
    ...
    hetero_data['paper'].n_id = paddle.arange(data.num_nodes)
...
def test_hgt_loader_disconnected():
    ...
    data['paper'].x = paddle.randn(shape=[10, 16])
    data['author'].x = paddle.randn(shape=[10, 16])

    # Paper nodes are disconnected from author nodes:
    data['paper', 'paper'].edge_index = get_random_edge_index(10, 10, 15)
    data['paper', 'paper'].edge_attr = paddle.randn(shape=[15, 8])
    data['author', 'author'].edge_index = get_random_edge_index(10, 10, 15)
    data['author', 'author'].edge_attr = paddle.randn(shape=[15, 8])
    ...
    for batch in loader:
        ...
        assert batch['author'].x.shape== (0, 16)
        assert batch['author', 'author'].num_edges == 0
        assert batch['author', 'author'].edge_index.shape== (2, 0)
        assert batch['author', 'author'].edge_attr.shape== (0, 8)
