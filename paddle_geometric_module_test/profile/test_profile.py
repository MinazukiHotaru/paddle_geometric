...
from paddle_geometric.profile.profile import paddle_profile, xpu_profile
...
def test_timeit(device):
    ...
    lin = paddle.nn.Linear(16, 32).to(device)
...
@withDevice
@onlyOnline
def test_paddle_profile(capfd, get_dataset, device):
    dataset = get_dataset(name='karate')
    data = dataset[0].to(device)
    model = GraphSAGE(dataset.num_features, hidden_channels=64, num_layers=3,
                      out_channels=dataset.num_classes).to(device)
