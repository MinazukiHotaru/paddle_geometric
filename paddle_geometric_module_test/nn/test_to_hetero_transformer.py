...
from paddle_geometric.nn import Linear as LazyLinear
from paddle_geometric.nn import (
    MeanAggregation,
    MessagePassing,
    RGCNConv,
...
class Net4(paddle.nn.Layer):
    ...
    def forward(self, x0: Tensor, edge_index: Tensor) -> Tensor:
        ...
        return self.lin1(paddle.concat([x0, x1, x2], dim=-1))


class Net5(paddle.nn.Layer):
...
class Net6(paddle.nn.Layer):
    def __init__(self, num_layers):
        super().__init__()
        self.lins = paddle.nn.LayerDict()
        self.convs = paddle.nn.LayerDict()
...
class Net11(paddle.nn.Layer):
    ...
    def forward(self, x: Tensor, edge_index: Tensor) -> Tensor:
        ...
        return paddle.concat(xs, dim=-1)


class Net12(paddle.nn.Layer):
    def __init__(self):
        super().__init__()
        self.conv = Net8()
...
def test_to_hetero_basic():
    x_dict = {
        'paper': paddle.randn(shape=[100, 16]),
        'author': paddle.randn(shape=[100, 16]),
    }
    edge_index_dict = {
        ('paper', 'cites', 'paper'):
        paddle.randint(100, (2, 200), dtype=paddle.int64),
    ...
    edge_attr_dict = {
        ('paper', 'cites', 'paper'): paddle.randn(shape=[200, 8]),
        ('paper', 'written_by', 'author'): paddle.randn(shape=[200, 8]),
        ('author', 'writes', 'paper'): paddle.randn(shape=[200, 8]),
    }
    ...
    assert out[0]['paper'].shape== (100, 32)
    assert out[0]['author'].shape== (100, 32)
    assert isinstance(out[1], dict) and len(out[1]) == 3
    assert out[1][('paper', 'cites', 'paper')].shape== (200, 16)
    assert out[1][('paper', 'written_by', 'author')].shape== (200, 16)
    assert out[1][('author', 'writes', 'paper')].shape== (200, 16)
    ...
    for aggr in ['sum', 'mean', 'min', 'max', 'mul']:
        ...
        assert out1['paper'].shape== (100, 32)
        assert out1['author'].shape== (100, 32)

        if paddle_geometric.typing.WITH_PADDLE_SPARSE:
            out2 = model(x_dict, adj_t_dict)
            assert isinstance(out2, dict) and len(out2) == 2
            for key in x_dict.keys():
                assert paddle.allclose(out1[key], out2[key], atol=1e-6)

    model = Net3()
    model = to_hetero(model, metadata, debug=False)
    out = model(x_dict, edge_index_dict, edge_attr_dict)
    assert isinstance(out, dict) and len(out) == 2
    assert out['paper'].shape== (100, 32)
    assert out['author'].shape== (100, 32)
    ...
    out = model(x_dict, edge_index_dict)
    assert isinstance(out, dict) and len(out) == 2
    assert out['paper'].shape== (100, 16)
    assert out['author'].shape== (100, 16)
    ...
    model = Net8()
    model = to_hetero(model, metadata, debug=False)
    out = model({'paper': paddle.randn(shape=[4, 8]), 'author': paddle.randn(shape=[8, 16])})
    assert isinstance(out, dict) and len(out) == 2
    assert out['paper'].shape== (4, 32)
    assert out['author'].shape== (8, 32)

    model = Net9()
    model = to_hetero(model, metadata, debug=False)
    out = model({'paper': paddle.randn(shape=[4, 16]), 'author': paddle.randn(shape=[8, 16])})
    assert isinstance(out, dict) and len(out) == 2
    assert out['paper'].shape== (4, 16)
    assert out['author'].shape== (8, 16)
    ...
    out = model(x_dict, edge_index_dict)
    assert isinstance(out, dict) and len(out) == 2
    assert out['paper'].shape== (100, 64)
    assert out['author'].shape== (100, 64)
...
def test_to_hetero_with_gcn():
    ...
    edge_index_dict = {
        ('paper', 'cites', 'paper'): paddle.randint(100, (2, 200)),
        ('paper', 'rev_cites', 'paper'): paddle.randint(100, (2, 200)),
    }
    metadata = list(x_dict.keys()), list(edge_index_dict.keys())
...
def test_to_hetero_and_rgcn_equal_output():
    ...
    # Run `RGCN`:
    x = paddle.randn(shape=[10, 16])  # 6 paper nodes, 4 author nodes
    adj = (paddle.rand(10, 10) > 0.5)
    adj[6:, 6:] = False
    edge_index = adj.nonzero(as_tuple=False).t().contiguous()
    row, col = edge_index

    # # 0 = paper<->paper, 1 = paper->author, 2 = author->paper
    edge_type = paddle.full((edge_index.shape[1], ), -1, dtype=paddle.int64)
    ...
    edge_index_dict = {
        ...
        ('paper', '_', 'author'):
        edge_index[:, edge_type == 1] - paddle.to_tensor([[0], [6]]),
        ('author', '_', 'paper'):
        edge_index[:, edge_type == 2] - paddle.to_tensor([[6], [0]]),
    }
    ...
    out2 = paddle.concat([out2['paper'], out2['author']], dim=0)
    assert paddle.allclose(out1, out2, atol=1e-6)

    if paddle_geometric.typing.WITH_PADDLE_SPARSE:
        out3 = model(x_dict, adj_t_dict)
        out3 = paddle.concat([out3['paper'], out3['author']], dim=0)
        assert paddle.allclose(out1, out3, atol=1e-6)


class GraphLevelGNN(paddle.nn.Layer):
    def __init__(self):
        super().__init__()
        self.conv = SAGEConv(16, 32)
...
def test_graph_level_to_hetero():
    ...
    batch_dict = {
        'paper': paddle.zeros(100, dtype=paddle.int64),
        'author': paddle.zeros(100, dtype=paddle.int64),
    }
...
class ModelLoops(paddle.nn.Layer):
    def __init__(self):
        super().__init__()
...
def test_to_hetero_on_static_graphs():
    x_dict = {
        'paper': paddle.randn(shape=[4, 100, 16]),
        'author': paddle.randn(shape=[4, 100, 16]),
    }
    edge_index_dict = {
        ('paper', 'written_by', 'author'): paddle.randint(100, (2, 200)),
        ('author', 'writes', 'paper'): paddle.randint(100, (2, 200)),
    }
    ...
    assert out_dict['paper'].shape== (4, 100, 32)
    assert out_dict['author'].shape== (4, 100, 32)
