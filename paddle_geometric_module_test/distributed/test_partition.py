...
from paddle_geometric.testing import onlyDistributedTest, withMETIS
...
def test_partition_data(tmp_path):
    ...
    node_map_path = osp.join(tmp_path, 'node_map.pdparams')
    assert osp.exists(node_map_path)
    node_map = fs.paddle_load(node_map_path)
    assert node_map.numel() == data.num_nodes

    edge_map_path = osp.join(tmp_path, 'edge_map.pdparams')
    assert osp.exists(edge_map_path)
    edge_map = fs.paddle_load(edge_map_path)
    assert edge_map.numel() == data.num_edges

    meta_path = osp.join(tmp_path, 'META.json')
    assert osp.exists(meta_path)

    graph0_path = osp.join(tmp_path, 'part_0', 'graph.pdparams')
    assert osp.exists(graph0_path)
    graph0 = fs.paddle_load(graph0_path)
    assert len({'edge_id', 'row', 'col', 'size'} & set(graph0.keys())) == 4

    graph1_path = osp.join(tmp_path, 'part_1', 'graph.pdparams')
    assert osp.exists(graph1_path)
    graph1 = fs.paddle_load(graph1_path)
    assert len({'edge_id', 'row', 'col', 'size'} & set(graph1.keys())) == 4

    node_feats0_path = osp.join(tmp_path, 'part_0', 'node_feats.pdparams')
    assert osp.exists(node_feats0_path)
    node_feats0 = fs.paddle_load(node_feats0_path)

    node_feats1_path = osp.join(tmp_path, 'part_1', 'node_feats.pdparams')
    assert osp.exists(node_feats1_path)
    node_feats1 = fs.paddle_load(node_feats1_path)

    assert (node_feats0['feats']['x'].shape[0] +
            node_feats1['feats']['x'].shape[0] == data.num_nodes)
    assert paddle.equal(data.x[node_feats0['global_id']],
                       node_feats0['feats']['x'])
    assert paddle.equal(data.x[node_feats1['global_id']],
                       node_feats1['feats']['x'])
...
def test_partition_hetero_data(tmp_path):
    ...
    for edge_type, num_edges in data.num_edges_dict.items():
        ...
        edge_map_path = osp.join(tmp_path, 'edge_map', f'{edge_name}.pdparams')
        assert osp.exists(edge_map_path)
        edge_map = fs.paddle_load(edge_map_path)
        assert edge_map.numel() == num_edges

    for node_type, num_nodes in data.num_nodes_dict.items():
        node_map_path = osp.join(tmp_path, 'node_map', f'{node_type}.pdparams')
        assert osp.exists(node_map_path)
        node_map = fs.paddle_load(node_map_path)
        assert node_map.numel() == num_nodes

    for pid in range(num_parts):
        graph_path = osp.join(tmp_path, f'part_{pid}', 'graph.pdparams')
        assert osp.exists(graph_path)
        node_feats_path = osp.join(tmp_path, f'part_{pid}', 'node_feats.pdparams')
        assert osp.exists(node_feats_path)
        edge_feats_path = osp.join(tmp_path, f'part_{pid}', 'edge_feats.pdparams')
...
def test_partition_data_temporal(tmp_path):
    data = FakeDataset()[0]
    data.time = paddle.arange(data.num_nodes)
    ...
    assert paddle.equal(data.time, node_feats0['time'])
    assert paddle.equal(data.time, node_feats1['time'])
...
def test_partition_data_edge_level_temporal(tmp_path):
    ...
    data.edge_time = paddle.arange(data.num_edges)

    partitioner = Partitioner(data, num_parts=2, root=tmp_path)
    partitioner.generate_partition()

    edge_feats0_path = osp.join(tmp_path, 'part_0', 'edge_feats.pdparams')
    assert osp.exists(edge_feats0_path)
    edge_feats0 = fs.paddle_load(edge_feats0_path)

    edge_feats1_path = osp.join(tmp_path, 'part_1', 'edge_feats.pdparams')
    assert osp.exists(edge_feats1_path)
    edge_feats1 = fs.paddle_load(edge_feats1_path)

    assert paddle.equal(data.edge_time[edge_feats0['global_id']],
                       edge_feats0['edge_time'])
    assert paddle.equal(data.edge_time[edge_feats1['global_id']],
...
def test_partition_hetero_data_temporal(tmp_path):
    ...
    for key in data.node_types:
        data[key].time = paddle.arange(data[key].num_nodes)
    ...
    for key in data.node_types:
        assert paddle.equal(data[key].time, node_feats0[key]['time'])
        assert paddle.equal(data[key].time, node_feats1[key]['time'])
...
def test_partition_hetero_data_edge_level_temporal(tmp_path):
    ...
    for key in data.edge_types:
        data[key].edge_time = paddle.arange(data[key].num_edges)
...
def test_from_partition_data(tmp_path):
    ...
    assert row1.shape[0] + row2.shape[0] == data.num_edges
    ...
    assert x1.shape[0] + x2.shape[0] == data.num_nodes
    assert paddle.allclose(data.x[id1], x1)
    assert paddle.allclose(data.x[id2], x2)
...
def test_from_partition_temporal_data(tmp_path):
    ...
    assert time1.shape[0] == data.num_nodes
    assert time2.shape[0] == data.num_nodes
    assert paddle.equal(time1, data.time)
    assert paddle.equal(time2, data.time)
...
def test_from_partition_edge_level_temporal_data(tmp_path):
    ...
    assert time1.shape[0] + time2.shape[0] == data.edge_index.shape[1]
    assert paddle.equal(data.edge_time[edge_id1], time1)
    assert paddle.equal(data.edge_time[edge_id2], time2)
...
def test_from_partition_hetero_temporal_data(tmp_path):
    ...
    for key in data.node_types:
        assert times1[key].shape[0] == data[key].num_nodes
        assert times2[key].shape[0] == data[key].num_nodes
        assert paddle.equal(times1[key], data[key].time)
        assert paddle.equal(times2[key], data[key].time)
...
def test_from_partition_hetero_edge_level_temporal_data(tmp_path):
    ...
    for key in data.edge_types:
        ...
        assert times1[key].shape[0] + times2[key].shape[0] == data[key].num_edges
        assert paddle.equal(data[key].edge_time[edge_id1], times1[key])
        assert paddle.equal(data[key].edge_time[edge_id2], times2[key])
