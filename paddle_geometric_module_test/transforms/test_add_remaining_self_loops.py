...
from paddle_geometric.transforms import AddRemainingSelfLoops
...
def test_add_remaining_self_loops():
    ...
    edge_weight = paddle.to_tensor([1, 2, 3, 4], dtype='float32')
    edge_attr = paddle.to_tensor([[1, 2], [3, 4], [5, 6], [7, 8]], dtype='float32')
    ...
    edge_index = paddle.to_tensor([[0, 0, 1, 2], [1, 0, 2, 1]])
    data = Data(edge_index=edge_index, num_nodes=3)
    data = AddRemainingSelfLoops()(data)
    assert len(data) == 2
...
def test_add_remaining_self_loops_all_loops_exist():
    ...
    # multiple times.
    edge_index = paddle.to_tensor([[0, 0, 1, 1, 2], [0, 0, 1, 1, 2]])
    data = Data(edge_index=edge_index, num_nodes=3)
    data = AddRemainingSelfLoops()(data)
