...
from paddle_geometric.graphgym import register
from paddle_geometric.graphgym.checkpoint import get_ckpt_dir
from paddle_geometric.graphgym.config import (
    cfg,
    dump_cfg,
    load_cfg,
    set_out_dir,
...
from paddle_geometric.graphgym.logger import LoggerCallback, set_printing
from paddle_geometric.graphgym.model_builder import create_model
from paddle_geometric.graphgym.models.gnn import FeatureEncoder, GNNStackStage
from paddle_geometric.graphgym.models.head import GNNNodeHead
from paddle_geometric.graphgym.train import GraphGymDataModule, train
from paddle_geometric.graphgym.utils import (
    agg_runs,
    auto_select_device,
    params_count,
)
from paddle_geometric.testing import onlyLinux, onlyOnline, withPackage
...
def test_run_single_graphgym(tmp_path, capfd, auto_resume, skip_train_eval,
    ...
    assert isinstance(optimizer[0], paddle.optim.Adam)
    assert isinstance(scheduler[0], paddle.optim.lr_scheduler.CosineAnnealingLR)
...
def test_graphgym_module(tmp_path):
    ...
    assert isinstance(outputs["loss"], paddle.to_tensor)
