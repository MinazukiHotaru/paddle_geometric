...
from paddle_geometric.transforms import ToUndirected
...
def test_to_undirected():
    ...
    perm = paddle.to_tensor([1, 2, 1, 2, 0, 0])
...
def test_to_undirected_with_duplicates():
    edge_index = paddle.to_tensor([[0, 0, 1, 1], [0, 1, 0, 2]])
    edge_weight = paddle.ones(4)
...
def test_hetero_to_undirected():
    edge_index = paddle.to_tensor([[2, 0], [3, 1]])
    edge_weight = paddle.randn(shape=[edge_index.shape[1]])
    edge_attr = paddle.randn(shape=[edge_index.shape[1], 8])

    perm = paddle.to_tensor([1, 1, 0, 0])
