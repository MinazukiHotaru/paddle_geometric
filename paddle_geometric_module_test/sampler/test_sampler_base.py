...
from paddle_geometric.utils import is_undirected
...
def test_homogeneous_to_bidirectional():
    ...
    obj = SamplerOutput(
        node=paddle.arange(10),
        row=edge_index[0],
        col=edge_index[0],
        edge=paddle.arange(edge_index.shape[1]),
    ).to_bidirectional()

    assert is_undirected(paddle.stack([obj.row, obj.col], dim=0))
...
def test_heterogeneous_to_bidirectional():
    ...
    assert is_undirected(
        paddle.stack([obj.row['v1', 'to', 'v1'], obj.col['v1', 'to', 'v1']], 0))
