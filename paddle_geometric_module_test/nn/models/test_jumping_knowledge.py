...
from paddle_geometric.nn import HeteroJumpingKnowledge, JumpingKnowledge
from paddle_geometric.testing import is_full_test


def test_jumping_knowledge():
    num_nodes, channels, num_layers = 100, 17, 5
    xs = list([paddle.randn(shape=[num_nodes, channels]) for _ in range(num_layers)])

    model = JumpingKnowledge('cat')
    assert str(model) == 'JumpingKnowledge(cat)'

    out = model(xs)
    assert out.shape== (num_nodes, channels * num_layers)

    if is_full_test():
        jit = paddle.jit.to_static(model)
        assert paddle.allclose(jit(xs), out)
...
def test_hetero_jumping_knowledge():
    ...
    types = ["author", "paper"]
    xs_dict = {
        key: [paddle.randn(shape=[num_nodes, channels]) for _ in range(num_layers)]
        for key in types
    }
    ...
    if is_full_test():
        ...
        jit_out = jit(xs_dict)
        for key in types:
            assert paddle.allclose(jit_out[key], out_dict[key])
