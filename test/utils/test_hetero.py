...
def test_construct_bipartite_edge_index():
    ...
    edge_attr_dict = {
        ('author', 'paper'): paddle.randn(edge_index.size(1), 16),
        ('paper', 'author'): paddle.randn(edge_index.size(1), 16)
    }
    ...
    assert paddle.equal_all(edge_attr[:20], edge_attr_dict['author',
                                                           'paper']).item()
    assert paddle.equal_all(edge_attr[20:], edge_attr_dict['paper',
                                                           'author']).item()
