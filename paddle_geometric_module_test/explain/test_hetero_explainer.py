...
from paddle_geometric.explain.config import ExplanationType


class DummyModel(paddle.nn.Layer):
    def forward(self, x_dict, edge_index_dict, *args) -> paddle.to_tensor:
...
@pytest.mark.parametrize('target', [None, paddle.randn(shape=[2])])
...
def test_forward(hetero_data, target, explanation_type):
    ...
    if target is None and explanation_type == ExplanationType.phenomenon:
        ...
        for key in explanation.node_types:
            ...
            assert explanation[key].node_mask.shape== expected_size
