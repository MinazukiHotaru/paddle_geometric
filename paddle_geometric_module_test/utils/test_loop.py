...
def test_remove_self_loops():
    ...
    assert out[0].equal(expected)
    assert out[1].equal(paddle.to_tensor([[1, 2], [3, 4]]))

    adj = to_paddle_coo_tensor(edge_index)
    adj, _ = remove_self_loops(adj)
    assert paddle.diag(adj.to_dense()).tolist() == [0, 0]
...
def test_segregate_self_loops():
    ...
    assert out[0].equal(paddle.to_tensor([[0, 1], [1, 0]]))
    assert out[1] is None
    assert out[2].equal(paddle.to_tensor([[0], [0]]))
    assert out[3] is None

    edge_attr = paddle.to_tensor([1, 2, 3])
    out = segregate_self_loops(edge_index, edge_attr)
    assert out[0].equal(paddle.to_tensor([[0, 1], [1, 0]]))
    assert out[1].equal(paddle.to_tensor([2, 3]))
    assert out[2].equal(paddle.to_tensor([[0], [0]]))
    assert out[3].equal(paddle.to_tensor([1]))
...
def test_add_self_loops():
    ...
    assert out[0].equal(expected)
    assert out[1].equal(paddle.to_tensor([0.5, 0.5, 0.5, 1., 1.]))

    out = add_self_loops(adj)[0]
    assert out._indices().equal(paddle.to_tensor([[0, 0, 1, 1], [0, 1, 0, 1]]))
    assert out._values().equal(paddle.to_tensor([1.5, 0.5, 0.5, 1.0]))

    out = add_self_loops(edge_index, edge_weight, fill_value=5)
    assert out[0].equal(expected)
    assert out[1].equal(paddle.to_tensor([0.5, 0.5, 0.5, 5.0, 5.0]))

    out = add_self_loops(adj, fill_value=5)[0]
    assert out._indices().equal(paddle.to_tensor([[0, 0, 1, 1], [0, 1, 0, 1]]))
    assert out._values().equal(paddle.to_tensor([5.5, 0.5, 0.5, 5.0]))

    out = add_self_loops(edge_index, edge_weight, fill_value=paddle.to_tensor(2.))
    assert out[0].equal(expected)
    assert out[1].equal(paddle.to_tensor([0.5, 0.5, 0.5, 2., 2.]))

    out = add_self_loops(adj, fill_value=paddle.to_tensor(2.))[0]
    assert out._indices().equal(paddle.to_tensor([[0, 0, 1, 1], [0, 1, 0, 1]]))
    assert out._values().equal(paddle.to_tensor([2.5, 0.5, 0.5, 2.0]))

    out = add_self_loops(edge_index, edge_weight, fill_value='add')
    assert out[0].equal(expected)
    assert out[1].equal(paddle.to_tensor([0.5, 0.5, 0.5, 1, 0.5]))
    ...
    edge_weight = paddle.empty(0)
    out = add_self_loops(edge_index, edge_weight, num_nodes=1)
    assert out[0].equal(paddle.to_tensor([[0], [0]]))
    assert out[1].equal(paddle.to_tensor([1.]))
...
def test_add_remaining_self_loops():
    ...
    assert out[0].equal(expected)
    assert out[1].equal(paddle.to_tensor([0.5, 0.5, 0.5, 1]))

    out = add_remaining_self_loops(edge_index, edge_weight, fill_value=5)
    assert out[0].equal(expected)
    assert out[1].equal(paddle.to_tensor([0.5, 0.5, 0.5, 5.0]))

    out = add_remaining_self_loops(edge_index, edge_weight,
                                   fill_value=paddle.to_tensor(2.))
    assert out[0].equal(expected)
    assert out[1].equal(paddle.to_tensor([0.5, 0.5, 0.5, 2.0]))

    out = add_remaining_self_loops(edge_index, edge_weight, fill_value='add')
    assert out[0].equal(expected)
    assert out[1].equal(paddle.to_tensor([0.5, 0.5, 0.5, 0.5]))
...
def test_add_remaining_self_loops_without_initial_loops():
    ...
    out = add_remaining_self_loops(edge_index, edge_weight)
    assert out[0].equal(expected)
    assert out[1].equal(paddle.to_tensor([0.5, 0.5, 1, 1]))

    out = add_remaining_self_loops(edge_index, edge_weight, fill_value=5)
    assert out[0].equal(expected)
    assert out[1].equal(paddle.to_tensor([0.5, 0.5, 5.0, 5.0]))

    out = add_remaining_self_loops(edge_index, edge_weight,
                                   fill_value=paddle.to_tensor(2.0))
    assert out[0].equal(expected)
    assert out[1].equal(paddle.to_tensor([0.5, 0.5, 2.0, 2.0]))
...
def test_get_self_loop_attr():
    ...
    assert full_loop_weight.equal(paddle.to_tensor([0.5, 0.0]))

    full_loop_weight = get_self_loop_attr(edge_index, edge_weight, num_nodes=4)
    assert full_loop_weight.equal(paddle.to_tensor([0.5, 0.0, 0.0, 0.0]))

    full_loop_weight = get_self_loop_attr(edge_index)
    assert full_loop_weight.equal(paddle.to_tensor([1.0, 0.0]))

    edge_attr = paddle.to_tensor([[1.0, 0.0], [0.0, 1.0], [0.5, 1.0]])
    full_loop_attr = get_self_loop_attr(edge_index, edge_attr)
    assert full_loop_attr.equal(paddle.to_tensor([[0.5, 1.0], [0.0, 0.0]]))
