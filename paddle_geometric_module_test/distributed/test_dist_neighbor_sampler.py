...
from paddle_geometric.sampler import NeighborSampler, NodeSamplerInput
from paddle_geometric.sampler.neighbor_sampler import node_sample
...
def dist_neighbor_sampler(
    ...
    if rank == 0:  # Seed nodes:
        input_node = paddle.to_tensor([1, 6])
    else:
        input_node = paddle.to_tensor([4, 9])
...
def dist_neighbor_sampler_temporal(
    ...
    if rank == 0:  # Seed nodes:
        input_node = paddle.to_tensor([1, 6], dtype=paddle.int64)
    else:
        input_node = paddle.to_tensor([4, 9], dtype=paddle.int64)
...
def dist_neighbor_sampler_hetero(
    ...
    input_node = paddle.to_tensor([node_0, node_1], dtype=paddle.int64)
...
def test_dist_neighbor_sampler_temporal_hetero(
    ...
    data['v0'].time = paddle.full((data.num_nodes_dict['v0'], ), 1,
                                 dtype=paddle.int64)
    data['v1'].time = paddle.full((data.num_nodes_dict['v1'], ), 2,
                                 dtype=paddle.int64)
...
def test_dist_neighbor_sampler_edge_level_temporal_hetero(
    ...
    for i, edge_type in enumerate(data.edge_types):
        data[edge_type].edge_time = paddle.full(
            (data[edge_type].edge_index.shape[1], ), i, dtype=paddle.int64)
