...
from paddle_geometric.data import download_url, extract_gz
from paddle_geometric.loader import DataLoader
from paddle_geometric.testing import onlyOnline, withPackage
from paddle_geometric.transforms import ToSLIC
...
def test_to_superpixels(tmp_path):
    ...
    from paddlevision.datasets.mnist import (
        MNIST,
        read_image_file,
        read_label_file,
    )
    ...
    paddle.save(test_set, osp.join(processed_folder, 'training.pdparams'))
    paddle.save(test_set, osp.join(processed_folder, 'test.pdparams'))
    ...
    data, y = dataset[0]
    assert len(data) == 2
    assert data.pos.dim() == 2 and data.pos.shape[1] == 2
    ...
    for batch, y in loader:
        ...
        assert batch.pos.dim() == 2 and batch.pos.shape[1] == 2
        assert batch.x.dim() == 2 and batch.x.shape[1] == 1
        assert batch.batch.dim() == 1
        assert batch.ptr.dim() == 1
        assert batch.pos.shape[0] == batch.x.shape[0] == batch.batch.shape[0]
        assert y.tolist() == [7, 2]
        break
    ...
    assert data.x.dim() == 2 and data.x.shape[1] == 1
    assert data.pos.shape[0] == data.x.shape[0]
    assert data.seg.shape== [1, 28, 28]
    assert data.img.shape== [1, 1, 28, 28]
    assert data.seg.max().item() + 1 == data.x.shape[0]
    assert y == 7
    ...
    for batch, y in loader:
        ...
        assert batch.ptr.dim() == 1
        assert batch.pos.shape[0] == batch.x.shape[0] == batch.batch.shape[0]
        assert batch.seg.shape== [2, 28, 28]
        assert batch.img.shape== [2, 1, 28, 28]
        assert y.tolist() == [7, 2]
        break
