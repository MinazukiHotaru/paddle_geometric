...
def test_feature_store():
    ...
    tensor = paddle.to_tensor([[0.0, 0.0, 0.0], [1.0, 1.0, 1.0],
                               [2.0, 2.0, 2.0]])
    ...
    assert paddle.equal_all(store.get_tensor(attr), tensor).item()
    assert paddle.equal_all(
        store.get_tensor(group_name, attr_name, index=paddle.tensor([0, 2])),
        tensor[paddle.tensor([0, 2])],
    ).item()

    assert store.update_tensor(tensor + 1, attr)
    assert paddle.equal_all(store.get_tensor(attr), tensor + 1).item()
    ...
    assert str(view) == (
        "AttrView(store=MyFeatureStore(), "
        "attr=TensorAttr(group_name='A', attr_name='feat', "
        "index=Tensor(shape=[3], dtype=int64, place=Place(gpu:0), "
        "stop_gradient=True,\n       [0, 1, 2])))")
    ...
    assert paddle.equal_all(store[group_name, attr_name, index], tensor).item()
    assert paddle.equal_all(store[group_name, attr_name, None], tensor).item()
    assert paddle.equal_all(store[group_name, attr_name, :], tensor).item()
    assert paddle.equal_all(store[group_name][attr_name][:], tensor).item()
    assert paddle.equal_all(store[group_name].feat[:], tensor).item()
    assert paddle.equal_all(store.view().A.feat[:], tensor).item()
    ...
    assert paddle.equal_all(store[group_name](), tensor).item()
...
def test_feature_store_override():
    ...
    assert paddle.equal_all(store[attr_name, index], tensor).item()
    assert paddle.equal_all(store[attr_name][index], tensor).item()
    assert paddle.equal_all(store[attr_name][:], tensor).item()
    assert paddle.equal_all(store[attr_name, :], tensor).item()
