...
from paddle_geometric.datasets import graph_generator
...
def test_node_property_split(property_name):
    ...
    for name, ratio in zip([
        ...
        assert data[name].dtype == paddle.bool
        assert data[name].shape == [100, ]
    ...
    node_ids = paddle.to_tensor(node_ids)
    assert node_ids.numel() == paddle.unique(node_ids).numel() == 100
