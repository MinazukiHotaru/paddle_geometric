...
def test_compose():
    ...
    assert data.edge_index.shape == [2, 7]
...
def test_compose_filters():
    ...
    data1 = Data(x=paddle.arange(3))
    assert not filter_fn(data1)

    data2 = Data(x=paddle.arange(2), edge_index=paddle.to_tensor([
        [0, 0, 1],
        [0, 1, 1],
    ]))
    assert not filter_fn(data2)

    data3 = Data(x=paddle.arange(3), edge_index=paddle.to_tensor([
        [0, 0, 1],
        [0, 1, 1],
    ]))
