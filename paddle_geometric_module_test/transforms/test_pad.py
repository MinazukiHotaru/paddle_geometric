...
from paddle_geometric.transforms import Pad
from paddle_geometric.transforms.pad import (
    AttrNamePadding,
    EdgeTypePadding,
    NodeTypePadding,
    Padding,
...
def _generate_heterodata_nodes(
    data: HeteroData
) -> Generator[Tuple[NodeType, str, paddle.Tensor], None, None]:
...
def _generate_heterodata_edges(
    data: HeteroData
) -> Generator[Tuple[EdgeType, str, paddle.Tensor], None, None]:
...
def _check_homo_data_nodes(
    ...
    if is_mask_available:
        ...
        assert paddle.all(padded.pad_node_mask[:compare_pad_start_idx])
        assert not paddle.any(padded.pad_node_mask[compare_pad_start_idx:])
    ...
    for attr in _generate_homodata_node_attrs(original):
        ...
        if not isinstance(padded[attr], paddle.Tensor):
            continue
        ...
        assert all(
            i == pad_value
            for i in paddle.flatten(padded[attr][compare_pad_start_idx:]))

        # Check values in non-padded area.
        assert paddle.equal(original[attr],
...
def _check_homo_data_edges(
    ...
    # Check values in non-padded area.
    assert paddle.equal(original.edge_index,
                       padded.edge_index[:, :compare_pad_start_idx])

    if is_mask_available:
        assert padded.pad_edge_mask.numel() == padded.num_edges
        assert paddle.all(padded.pad_edge_mask[:compare_pad_start_idx])
        assert not paddle.any(padded.pad_edge_mask[compare_pad_start_idx:])
...
def _check_hetero_data_nodes(
    ...
    for node_type, attr in _generate_heterodata_nodes(original):
        ...
        if not isinstance(padded[node_type][attr], paddle.Tensor):
            continue
        ...
        if attr == 'pad_node_mask':
            ...
            assert paddle.all(padded_tensor[:compare_pad_start_idx])
            assert not paddle.any(padded_tensor[compare_pad_start_idx:])
            continue
        ...
        assert all(
            i == pad_value
            for i in paddle.flatten(padded_tensor[compare_pad_start_idx:]))
        # Compare non-padded area with the original.
        assert paddle.equal(original_tensor,
...
def _check_hetero_data_edges(
    ...
    for edge_type, attr in _generate_heterodata_edges(padded):
        ...
        if not isinstance(padded[edge_type][attr], paddle.Tensor):
            continue
        ...
        if attr == 'edge_index':
            ...
            assert all(
                i == src_nodes
                for i in paddle.flatten(padded_tensor[0,
                                                     compare_pad_start_idx:]))
            dst_nodes = original[edge_type[2]].num_nodes
            assert all(
                i == dst_nodes
                for i in paddle.flatten(padded_tensor[1,
                                                     compare_pad_start_idx:]))
            ...
            assert all(i == pad_value for i in paddle.flatten(padded_tensor[
                compare_pad_start_idx:, :]))
...
def test_pad_node_additional_attr_mask(mask_pad_value):
    data = fake_data()
    mask = paddle.randn(shape=[data.num_nodes]) > 0
    ...
    for padded_mask in padded_masks:
        ...
        assert paddle.all(padded_mask[-padding_num:] == mask_pad_value)
