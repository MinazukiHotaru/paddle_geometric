...
from paddle_geometric.transforms import RandomNodeSplit
...
def test_random_node_split(num_splits):
    ...
    y = paddle.randint(num_classes, shape=[num_nodes, ], dtype=paddle.int64)
    data = Data(x=x, y=y)
    ...
    assert train_mask.shape== [num_nodes, num_splits]
    val_mask = data.val_mask
    val_mask = val_mask.unsqueeze(-1) if num_splits == 1 else val_mask
    assert val_mask.shape== [num_nodes, num_splits]
    test_mask = data.test_mask
    test_mask = test_mask.unsqueeze(-1) if num_splits == 1 else test_mask
    assert test_mask.shape== [num_nodes, num_splits]
...
def test_random_node_split_on_hetero_data():
    ...
    data['paper'].x = paddle.randn(shape=[2000, 16])
    data['paper'].y = paddle.randint(4, (2000, ), dtype=paddle.int64)
    data['author'].x = paddle.randn(shape=[300, 16])
