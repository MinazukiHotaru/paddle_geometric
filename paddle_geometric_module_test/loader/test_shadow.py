...
def test_shadow_k_hop_sampler():
    ...
    edge_weight = paddle.arange(row.shape[0])
    x = paddle.randn(shape=[6, 16])
    y = paddle.randint(3, (6, ), dtype=paddle.int64)
    data = Data(edge_index=edge_index, edge_weight=edge_weight, x=x, y=y)

    train_mask = paddle.to_tensor([1, 1, 0, 0, 0, 0], dtype=paddle.bool)
    ...
    assert batch1.x.tolist() == x[paddle.to_tensor([0, 1, 2, 3, 0, 1, 2])].tolist()
    ...
    e_id = paddle.to_tensor([0, 1, 2, 3, 4, 5, 6, 9, 0, 1, 3, 4, 5, 6])
