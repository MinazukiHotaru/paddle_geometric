...
from paddle_geometric.nn import GCN
from paddle_geometric.nn.model_hub import PyGModelHubMixin
...
def test_model_init():
    model = DummyModel(
        MODEL_NAME, DATASET_NAME, model_kwargs={
            **CONFIG, 'tensor': paddle.randn(shape=[[1, 2, 3]])
        })
...
def test_from_pretrained_internal(model, monkeypatch):
    ...
    monkeypatch.setattr('paddle_geometric.nn.model_hub.hf_hub_download',
                        hf_hub_download)
    monkeypatch.setattr('paddle_geometric.nn.model_hub.fs.paddle_load',
