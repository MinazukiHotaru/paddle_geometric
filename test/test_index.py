...
sys.path.append(
    "/root/host/home/zhangzhimin04/workspaces_123/geometric_padddle_torch/paddle_project"
)
import os
...
#     assert index._indptr.equal(paddle.to_tensor(data=[0, 1, 3, 4], place=device))
...
#     assert out.dim_size == 4
#     assert out._indptr.equal(paddle.to_tensor(data=[0, 1, 3, 4, 4], place=device))
#     out = index.dim_resize_(3)
#     assert out.dim_size == 3
#     assert out._indptr.equal(paddle.to_tensor(data=[0, 1, 3, 4], place=device))
...
#     assert isinstance(index, Index)
#     assert index.place == device
#     assert index._indptr.dtype == dtype
#     assert index._indptr.device == device
#     out = index.cpu()
#     assert isinstance(out, Index)
#     assert out.place == device2str("cpu")
#     out = index.to("int32")
#     assert out.dtype == "int32"
#     if paddle_geometric.typing.WITH_PT20:
#         assert isinstance(out, Index)
#         assert out._indptr.dtype == "int32"
#     else:
#         assert not isinstance(out, Index)
#     out = index.to("float32")
#     assert not isinstance(out, Index)
#     assert out.dtype == "float32"
#     out = index.astype(dtype="int64")
#     assert isinstance(out, Index)
#     assert out.dtype == "int64"
#     out = index.astype(dtype="int32")
#     assert out.dtype == "int32"
#     if paddle_geometric.typing.WITH_PT20:
#         assert isinstance(out, Index)
#     else:
...
#     index, _ = paddle.sort(x=index), paddle.argsort(x=index)

#     assert isinstance(index, Index)
#     assert index.equal_all(y=paddle.to_tensor(data=[0, 1, 1, 2], place=device)).item()
#     assert index.dim_size == 3
#     assert index.is_sorted
#     out, perm = paddle.sort(x=index), paddle.argsort(x=index)
#     assert isinstance(out, Index)
#     assert out._data.data_ptr() == index._data.data_ptr()
#     assert perm.equal_all(y=paddle.to_tensor(data=[0, 1, 2, 3], place=device)).item()
#     assert out.dim_size == 3
#     index, _ = paddle.sort(descending=True, x=index), paddle.argsort(
#         descending=True, x=index
#     )
#     assert isinstance(index, Index)
#     assert index.equal_all(y=paddle.to_tensor(data=[2, 1, 1, 0], place=device)).item()
#     assert index.dim_size == 3
#     assert not index.is_sorted
...
#     kwargs = dict(dtype=dtype, device=device)
#     index = Index([1, 0, 2, 1], dim_size=3, **kwargs)
#     index, perm = paddle.sort(stable=True, x=index), paddle.argsort(
#         stable=True, x=index
#     )
#     assert isinstance(index, Index)
#     assert index.equal_all(y=paddle.to_tensor(data=[0, 1, 1, 2], place=device)).item()
...
#     assert index.dim_size == 3
#     assert index.is_sorted
#     out, perm = paddle.sort(stable=True, x=index), paddle.argsort(stable=True, x=index)
#     assert isinstance(out, Index)
#     assert out._data.data_ptr() == index._data.data_ptr()
#     assert perm.equal_all(y=paddle.to_tensor(data=[0, 1, 2, 3], place=device)).item()
#     assert out.dim_size == 3
#     index, perm = paddle.sort(descending=True, stable=True, x=index), paddle.argsort(
#         descending=True, stable=True, x=index
#     )
#     assert isinstance(index, Index)
#     assert index.equal_all(y=paddle.to_tensor(data=[2, 1, 1, 0], place=device)).item()
...
def test_narrow(dtype, device):
    kwargs = dict(dtype=dtype, device=device)
    index = Index([0, 1, 1, 2], dim_size=3, is_sorted=True, **kwargs)
    start_2 = index.shape[0] + 1 if 1 < 0 else 1
    out = paddle.slice(index, [0], [start_2], [start_2 + 2])
    assert isinstance(out, Index)
...
def test_getitem(dtype, device):
    ...
    assert isinstance(out, Index)
    assert out._data.data_ptr() == index._data.data_ptr()
    assert out.equal_all(y=paddle.to_tensor(data=[0, 1, 1, 2], place=device)).item()
    assert out.dim_size == 3
    assert out.is_sorted
    out = index[paddle.to_tensor(data=[False, True, False, True], place=device)]
    assert isinstance(out, Index)
    assert out.equal_all(y=paddle.to_tensor(data=[1, 2], place=device)).item()
    assert out.dim_size == 3
    assert out.is_sorted
    ...
    out = index[1:3]
    assert isinstance(out, Index)
    assert out.equal_all(y=paddle.to_tensor(data=[1, 1], place=device)).item()
    assert out.dim_size == 3
    assert out.is_sorted
    out = index[...]
    assert isinstance(out, Index)
    ...
    out = index[0]
    assert not isinstance(out, Index)
    assert out.equal_all(y=paddle.to_tensor(data=0, place=device)).item()
...
def test_add(dtype, device):
    kwargs = dict(dtype=dtype, device=device)
    index = Index([0, 1, 1, 2], dim_size=3, is_sorted=True, **kwargs)
    out = paddle.add(x=index, y=paddle.to_tensor(2 * 2))
    assert isinstance(out, Index)
    assert out.equal_all(y=paddle.to_tensor(data=[4, 5, 5, 6], place=device)).item()
    assert out.dim_size == 7
    assert out.is_sorted
    out = index + paddle.to_tensor(data=[2], dtype=dtype, place=device)
    assert isinstance(out, Index)
    assert out.equal_all(y=paddle.to_tensor(data=[2, 3, 3, 4], place=device)).item()
    ...
    out = index.add(index)
    assert isinstance(out, Index)
    assert out.equal_all(y=paddle.to_tensor(data=[0, 2, 2, 4], place=device)).item()
    assert out.dim_size == 6
    assert not out.is_sorted
    index += 2
    assert isinstance(index, Index)
    assert index.equal_all(y=paddle.to_tensor(data=[2, 3, 3, 4], place=device)).item()
...
def test_sub(dtype, device):
    ...
    out = paddle.subtract(x=index, y=paddle.to_tensor(2 * 2))
    assert isinstance(out, Index)
    assert out.equal_all(y=paddle.to_tensor(data=[0, 1, 1, 2], place=device)).item()
    assert out.dim_size == 3
    assert out.is_sorted
    ...
    assert isinstance(out, Index)
    assert out.equal_all(y=paddle.to_tensor(data=[2, 1, 1, 0], place=device)).item()
    assert out.dim_size is None
    assert not out.is_sorted
    out = index.sub(index)
    assert isinstance(out, Index)
    assert out.equal_all(y=paddle.to_tensor(data=[0, 0, 0, 0], place=device)).item()
    assert out.dim_size is None
    assert not out.is_sorted
    index -= 2
    assert isinstance(index, Index)
