...
import paddle_geometric.distributed.rpc as rpc
...
from paddle_geometric.distributed.rpc import RPCRouter
...
def run_rpc_feature_test(
    ...
    partition_book: paddle.to_tensor,
    master_port: int,
):
    ...
    # Global node IDs:
    global_id0 = paddle.arange(128 * 2)
    global_id1 = paddle.arange(128 * 2) + 128 * 2
    ...
    cpu_tensor0 = paddle.concat([paddle.ones(128, 1024), paddle.ones(128, 1024) * 2])
    cpu_tensor1 = paddle.concat([paddle.zeros(128, 1024), paddle.zeros(128, 1024)])

    # Verify..
    assert paddle.allclose(cpu_tensor0, tensor0.wait())
    assert paddle.allclose(cpu_tensor1, tensor1.wait())
...
def test_dist_feature_lookup():
    ...
    partition_book = paddle.concat([
        paddle.zeros(128 * 2, dtype=paddle.int64),
        paddle.ones(128 * 2, dtype=paddle.int64),
    ])
