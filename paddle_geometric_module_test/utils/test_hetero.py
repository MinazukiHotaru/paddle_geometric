...
def test_construct_bipartite_edge_index():
    ...
    edge_attr_dict = {
        ('author', 'paper'): paddle.randn(shape=[edge_index.shape[1], 16]),
        ('paper', 'author'): paddle.randn(shape=[edge_index.shape[1], 16])
    }
    ...
    assert edge_attr.shape == [40, 16]
    assert paddle.equal_all(edge_attr[:20], edge_attr_dict['author', 'paper'])
    assert paddle.equal_all(edge_attr[20:], edge_attr_dict['paper', 'author'])
