...
from paddle_geometric.nn.pool.select import SelectOutput, SelectTopK
from paddle_geometric.nn.pool.select.topk import topk
from paddle_geometric.profile import benchmark
from paddle_geometric.testing import is_full_test


def test_topk_ratio():
    x = paddle.to_tensor([2.0, 4.0, 5.0, 6.0, 2.0, 9.0])
    batch = paddle.to_tensor([0, 0, 1, 1, 1, 1])
    ...
    if is_full_test():
        jit = paddle.jit.to_static(topk)
        assert paddle.equal(jit(x, 0.5, batch), perm1)
        assert paddle.equal(jit(x, 2, batch), perm2)
        assert paddle.equal(jit(x, 3, batch), perm3)
...
if __name__ == '__main__':
    ...
    for B, N in product(BS, NS):
        x = paddle.randn(shape=[N, place=args.device])
        batch = paddle.randint(0, B, (N, ), place=args.device).sort()[0]
