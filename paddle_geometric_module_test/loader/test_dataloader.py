...
from paddle_geometric.data import Data, HeteroData, OnDiskDataset
...
def test_dataloader(num_workers, device):
    if num_workers > 0 and device != paddle.place('cpu'):
        return
...
def test_dataloader_on_disk_dataset(tmp_path, num_workers):
    ...
    data1 = Data(x=paddle.randn(shape=[3, 8]))
    data2 = Data(x=paddle.randn(shape=[4, 8]))
    ...
    assert paddle.equal(batch.x, paddle.concat([data1.x, data2.x], dim=0))
...
def test_dataloader_fallbacks():
    # Test inputs of type List[paddle.to_tensor]:
    data_list = [paddle.ones(3) for _ in range(4)]
    batch = next(iter(DataLoader(data_list, batch_size=4)))
    assert paddle.equal(batch, paddle.ones(4, 3))

    # Test inputs of type List[float]:
    data_list = [1.0, 1.0, 1.0, 1.0]
    batch = next(iter(DataLoader(data_list, batch_size=4)))
    assert paddle.equal(batch, paddle.ones(4))

    # Test inputs of type List[int]:
    data_list = [1, 1, 1, 1]
    batch = next(iter(DataLoader(data_list, batch_size=4)))
    assert paddle.equal(batch, paddle.ones(4, dtype=paddle.int64))
    ...
    data_list = [{'x': paddle.ones(3), 'y': 1}] * 4
    batch = next(iter(DataLoader(data_list, batch_size=4)))
    assert paddle.equal(batch['x'], paddle.ones(4, 3))
    assert paddle.equal(batch['y'], paddle.ones(4, dtype=paddle.int64))

    # Test inputs of type List[Tuple]:
    DataTuple = namedtuple('DataTuple', 'x y')
    data_list = [DataTuple(0.0, 1)] * 4
    batch = next(iter(DataLoader(data_list, batch_size=4)))
    assert paddle.equal(batch.x, paddle.zeros(4))
    assert paddle.equal(batch[1], paddle.ones(4, dtype=paddle.int64))

    # Test inputs of type List[Sequence]:
    data_list = [[0.0, 1]] * 4
    batch = next(iter(DataLoader(data_list, batch_size=4)))
    assert paddle.equal(batch[0], paddle.zeros(4))
    assert paddle.equal(batch[1], paddle.ones(4, dtype=paddle.int64))
...
def test_multiprocessing():
    queue = paddle.multiprocessing.Manager().Queue()
...
def test_pin_memory():
    ...
    for batch in loader:
        assert batch.x.is_pinned() or not paddle.cuda.is_available()
        assert batch.edge_index.is_pinned() or not paddle.cuda.is_available()


@pytest.mark.parametrize('num_workers', num_workers_list)
def test_heterogeneous_dataloader(num_workers):
    data = HeteroData()
    data['p'].x = paddle.randn(shape=[100, 128])
    data['a'].x = paddle.randn(shape=[200, 128])
    data['p', 'a'].edge_index = get_random_edge_index(100, 200, 500)
    data['p'].edge_attr = paddle.randn(shape=[500, 32])
    data['a', 'p'].edge_index = get_random_edge_index(200, 100, 400)
    data['a', 'p'].edge_attr = paddle.randn(shape=[400, 32])
...
def test_index_dataloader(num_workers):
    ...
    for batch in loader:
        ...
        assert batch.index.dtype == paddle.int64
...
def test_edge_index_dataloader(num_workers, sort_order):
    ...
    for batch in loader:
        ...
        assert batch.edge_index.dtype == paddle.int64
...
def test_dataloader_sparse():
    adj_t = paddle.sparse_coo_tensor(
        indices=paddle.to_tensor([[0, 1, 1, 2], [1, 0, 2, 1]]),
        values=paddle.randn(shape=[4]),
        size=(3, 3),
    )
