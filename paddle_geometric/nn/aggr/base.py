...
class Aggregation(paddle.nn.Layer):
    ...
    def __init__(self) -> None:
        ...
        self._deterministic: Final[bool] = False  # 或者根据需求设置为 True
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if index is None and ptr is None:
            index = x.new_zeros(x.size(dim), dtype=paddle.int64)
    ...
    def assert_sorted_index(self, index: Optional[Tensor]):
        if index is not None and not paddle.all(index[:-1] <= index[1:]):
