...
def test_unbatch():
    ...
    for i in range(len(out)):
        expected = src[paddle.nonzero(batch == i).flatten()]
        assert paddle.all(out[i] == expected).item()
...
def test_unbatch_edge_index():
    ...
    assert edge_indices[0].numpy().tolist() == [[0, 1, 1, 2, 2, 3], [1, 0, 2, 1, 3, 2]]
    assert edge_indices[1].numpy().tolist() == [[0, 1, 1, 2], [1, 0, 2, 1]]
