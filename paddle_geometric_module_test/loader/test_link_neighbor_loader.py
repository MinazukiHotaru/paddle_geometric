...
from paddle_geometric.loader import LinkNeighborLoader
...
def test_homo_link_neighbor_loader_basic(device, subgraph_type,
                                         neg_sampling_ratio,
                                         filter_per_worker):
    pos_edge_index = get_random_edge_index(50, 50, 500, place=device)
    neg_edge_index = get_random_edge_index(50, 50, 500, place=device)
    neg_edge_index += 50

    input_edges = paddle.concat([pos_edge_index, neg_edge_index], dim=-1)
    edge_label = paddle.concat([
        paddle.ones(500, place=device),
        paddle.zeros(500, place=device),
    ], dim=0)
    ...
    for batch in loader:
        ...
        if neg_sampling_ratio is None:
            ...
            assert paddle.all(batch.edge_label[:20] == 1)
            assert paddle.all(batch.edge_label[20:] == 0)
...
def test_hetero_link_neighbor_loader_basic(subgraph_type, neg_sampling_ratio):
    ...
    for batch in loader:
        ...
        if neg_sampling_ratio is None:
            ...
            assert batch['paper', 'author'].edge_label_index.shape[1] == 40
            assert paddle.all(batch['paper', 'author'].edge_label[:20] == 1)
            assert paddle.all(batch['paper', 'author'].edge_label[20:] == 0)
...
def test_link_neighbor_loader_edge_label():
    ...
    data = Data(edge_index=edge_index, x=paddle.arange(100))
    ...
    for batch in loader:
        assert batch.edge_label.dtype == paddle.float32
        assert paddle.all(batch.edge_label[:10] == 1.0)
        assert paddle.all(batch.edge_label[10:] == 0.0)

    loader = LinkNeighborLoader(
        data,
        num_neighbors=[-1] * 2,
        batch_size=10,
        edge_label=paddle.ones(500, dtype=paddle.int64),
        neg_sampling_ratio=1.0,
    )

    for batch in loader:
        assert batch.edge_label.dtype == paddle.int64
        assert paddle.all(batch.edge_label[:10] == 1)
        assert paddle.all(batch.edge_label[10:] == 0)
...
def test_temporal_homo_link_neighbor_loader(batch_size):
    data = Data(
        x=paddle.randn(shape=[10, 5]),
        edge_index=paddle.randint(0, 10, (2, 123)),
        time=paddle.arange(10),
    )

    # Ensure that nodes exist at the time of the `edge_label_time`:
    edge_label_time = paddle.max(
    ...
    loader = LinkNeighborLoader(
        ...
        num_neighbors=[-1],
        time_attr='time',
        edge_label=paddle.ones(data.num_edges),
    ...
    for batch in loader:
        assert batch.edge_label_index.shape== (2, batch_size)
        assert batch.edge_label_time.shape== (batch_size, )
        assert batch.edge_label.shape== (batch_size, )
        assert paddle.all(batch.time <= batch.edge_label_time)
...
def test_temporal_hetero_link_neighbor_loader():
    ...
    data['paper'].x = paddle.arange(100)
    data['paper'].time = paddle.arange(data['paper'].num_nodes) - 200
    data['author'].x = paddle.arange(100, 300)
    ...
    # With edge_time:
    edge_time = paddle.arange(data['paper', 'paper'].edge_index.shape[1])
    loader = LinkNeighborLoader(
        data,
        num_neighbors=[-1] * 2,
        edge_label_index=('paper', 'paper'),
...
def test_custom_hetero_link_neighbor_loader():
    ...
    for (batch1, batch2) in zip(loader1, loader2):
        ...
        assert paddle.allclose(batch1['paper'].x.sort()[0],
                              batch2['paper'].x.sort()[0])
        assert paddle.allclose(batch1['author'].x.sort()[0],
                              batch2['author'].x.sort()[0])

        # Assert that edge indices have the same size:
        assert (batch1['paper', 'to', 'paper'].edge_index.shape== batch1[
            'paper', 'to', 'paper'].edge_index.size())
        assert (batch1['paper', 'to', 'author'].edge_index.shape== batch1[
            'paper', 'to', 'author'].edge_index.size())
        assert (batch1['author', 'to', 'paper'].edge_index.shape== batch1[
...
def test_hetero_link_neighbor_loader_no_edges():
    ...
    for batch in loader:
        ...
        assert batch['paper', 'paper'].edge_label_index.shape[1] == 20
...
def test_homo_link_neighbor_loader_triplet(disjoint, temporal, amount):
    ...
    if temporal:
        ...
        edge_label_time = paddle.max(data.time[data.edge_label_index[0]],
    ...
    for batch in loader:
        ...
        assert paddle.equal(batch.x[batch.src_index],
                           data.edge_label_index[0, batch.input_id])
        assert paddle.equal(batch.x[batch.dst_pos_index],
                           data.edge_label_index[1, batch.input_id])

        # Check that `dst_neg_index` points to valid nodes in the batch:
        if amount == 1:
            assert batch.dst_neg_index.shape== (batch_size, )
        else:
            assert batch.dst_neg_index.shape== (batch_size, amount)
        ...
        if disjoint:
            ...
            for i in range(0, max_seed_nodes, batch_size):
                ...
                assert paddle.equal(batch_vector, paddle.arange(batch_size))
...
def test_hetero_link_neighbor_loader_triplet(disjoint, temporal, amount):
    ...
    weight = paddle.rand(data['paper'].num_nodes) if not temporal else None
    ...
    for batch in loader:
        ...
        # Check that `dst_neg_index` points to valid nodes in the batch:
        if amount == 1:
            assert node_store.dst_neg_index.shape== (batch_size, )
        else:
            assert node_store.dst_neg_index.shape== (batch_size, amount)
