...
def test_to_networkx_hetero():
    ...
    data['author', 'paper'].edge_attr = paddle.arange(edge_index.size(1))
...
def test_from_networkx():
    x = paddle.randn(2, 8)
    pos = paddle.randn(2, 3)
    edge_index = paddle.to_tensor([[0, 1, 0], [1, 0, 0]])
    edge_attr = paddle.randn(edge_index.size(1))
...
def test_from_networkx_group_attrs():
    ...
    x2 = paddle.randn(2, 8)
    edge_index = paddle.to_tensor([[0, 1, 0], [1, 0, 0]])
    edge_attr1 = paddle.randn(edge_index.size(1))
    edge_attr2 = paddle.randn(edge_index.size(1))
    perm = paddle.to_tensor([0, 2, 1])
    ...
    assert len(data) == 4
    assert data.x.tolist() == paddle.cat([x, x2], dim=-1).tolist()
...
def test_from_networkx_sbm(n, p, q):
    ...
    assert paddle.equal_all(data.block[:50], data.block.new_zeros(50)).item()
    assert paddle.equal_all(data.block[50:], data.block.new_ones(50)).item()
...
def test_to_networkit(directed, edge_weight, num_nodes):
    ...
    edge_index = paddle.to_tensor([[0, 1, 1], [1, 0, 2]], dtype=paddle.long)
...
def test_to_cugraph(edge_weight, directed, relabel_nodes):
    ...
    cu_edge_index = paddle.from_numpy(cu_edge_index).t()
    cu_edge_weight = None
    if edge_weight is not None:
        cu_edge_weight = edge_list['2'].to_pandas().values
        cu_edge_weight = paddle.from_numpy(cu_edge_weight)

    cu_edge_index, cu_edge_weight = sort_edge_index(cu_edge_index,
                                                    cu_edge_weight)

    assert paddle.equal_all(edge_index, cu_edge_index.cpu()).item()
    if edge_weight is not None:
...
@pytest.mark.skip("Not supported")
@withPackage('cudf', 'cugraph')
@pytest.mark.parametrize('edge_weight', [None, paddle.randn(4)])
@pytest.mark.parametrize('directed', [True, False])
@pytest.mark.parametrize('relabel_nodes', [True, False])
...
def test_to_dgl_graph():
    ...
    edge_attr = paddle.randn(edge_index.size(1), 2)
    data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr)

    g = to_dgl(data)

    assert paddle.equal_all(data.x, g.ndata['x'])
    row, col = g.edges()
    assert paddle.equal_all(row, edge_index[0])
    assert paddle.equal_all(col, edge_index[1])
    assert paddle.equal_all(data.edge_attr, g.edata['edge_attr'])
...
def test_to_dgl_hetero_graph():
    data = HeteroData()
    data['v1'].x = paddle.randn(4, 3)
    data['v2'].x = paddle.randn(4, 3)
    data['v1', 'v2'].edge_index = paddle.to_tensor([[0, 1, 2, 3], [0, 1, 2,
                                                                   3]])
    data['v1', 'v2'].edge_attr = paddle.randn(4, 2)
    ...
    assert paddle.equal_all(data['v1'].x, g.nodes['v1'].data['x'])
    assert paddle.equal_all(data['v2'].x, g.nodes['v2'].data['x'])
    row, col = g.edges()
    assert paddle.equal_all(row, data['v1', 'v2'].edge_index[0])
    assert paddle.equal_all(col, data['v1', 'v2'].edge_index[1])
    assert paddle.equal_all(g.edata['edge_attr'], data['v1', 'v2'].edge_attr)
...
def test_to_dgl_sparse():
    ...
    assert paddle.equal_all(pyg_row, dgl_row)
    assert paddle.equal_all(pyg_col, dgl_col)
    assert paddle.equal_all(data.edge_attr, g.edata['edge_attr'])
...
def test_from_dgl_graph():
    ...
    g.edata['edge_attr'] = paddle.randn(g.num_edges())

    data = from_dgl(g)

    assert paddle.equal_all(data.x, g.ndata['x'])
    row, col = g.edges()
    assert paddle.equal_all(data.edge_index[0], row)
    assert paddle.equal_all(data.edge_index[1], col)
    assert paddle.equal_all(data.edge_attr, g.edata['edge_attr'])
...
def test_from_dgl_hetero_graph():
    ...
    g.nodes['v1'].data['x'] = paddle.randn(5, 3)
    g.nodes['v2'].data['x'] = paddle.randn(3, 3)
