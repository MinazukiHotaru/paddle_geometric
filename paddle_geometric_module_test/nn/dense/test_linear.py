...
def test_lazy_linear(weight, bias, device):
    ...
    assert copied_lin(x).shape== (3, 4, 32)
...
def test_load_lazy_linear(dim1, dim2, bias, device):
    ...
    if dim1 != -1:
        assert isinstance(lin1.weight, paddle.nn.Parameter)
        assert isinstance(lin2.weight, paddle.nn.Parameter)
    ...
    if bias:
        assert isinstance(lin1.bias, paddle.nn.Parameter)
        assert isinstance(lin2.bias, paddle.nn.Parameter)
...
def test_identical_linear_default_initialization(lazy):
    ...
    assert paddle.equal(lin1.weight, lin2.weight)
    assert paddle.equal(lin1.bias, lin2.bias)
...
def test_hetero_linear_basic(device):
    x = paddle.randn(shape=[3, 16, place=device])
...
def test_hetero_linear_initializer():
    ...
    assert paddle.equal(lin.bias, paddle.zeros_like(lin.bias))
...
def test_hetero_linear_amp(device, use_segment_matmul):
    ...
    with paddle.amp.autocast('cuda'):
        assert lin(x, type_vec).shape== (3, 32)
...
def test_hetero_dict_linear(bias, device):
    x_dict = {
        'v': paddle.randn(shape=[3, 16, place=device]),
        'w': paddle.randn(shape=[2, 8, place=device]),
    }
    ...
    assert out_dict['v'].shape== (3, 32)
    assert out_dict['w'].shape== (2, 32)

    x_dict = {
        'v': paddle.randn(shape=[3, 16, place=device]),
        'w': paddle.randn(shape=[2, 16, place=device]),
    }
...
def test_hetero_dict_linear_jit():
    x_dict = {
        'v': paddle.randn(shape=[3, 16]),
        'w': paddle.randn(shape=[2, 8]),
    }
...
def test_hetero_linear_sort(type_vec, device):
    x = paddle.randn(shape=[type_vec.numel(]), 16, place=device)
...
if __name__ == '__main__':
    ...
    def get_xs(mean: float, std: float, num_types: int,
        ...
        num_nodes_list = paddle.normal(
            mean=paddle.to_tensor([mean] * num_types, dtype=paddle.float32),
            std=paddle.to_tensor([std] * num_types, dtype=paddle.float32),
        ).round().to(paddle.int64).tolist()

        return [
            paddle.randn(shape=[num_nodes, channels, place=args.device])
    ...
    for num_types in [3, 5, 10, 50, 100, 200, 500, 1000]:
        ...
        count = paddle.to_tensor([x.shape[0] for x in xs])
        ptr = cumsum(paddle.to_tensor([x.shape[0] for x in xs]))
        x = paddle.concat(xs, dim=0)
        padded_x = paddle.nested.nested_tensor(xs).to_padded_tensor(padding=0.0)
        weight = paddle.randn(shape=[num_types, channels, channels, place=args.device])
