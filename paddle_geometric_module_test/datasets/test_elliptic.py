from paddle_geometric.testing import onlyFullTest, onlyOnline
...
def test_elliptic_bitcoin_dataset(get_dataset):
    ...
    data = dataset[0]
    assert len(data) == 5
    assert data.x.shape== (203769, 165)
    assert data.edge_index.shape== (2, 234355)
    assert data.y.shape== (203769, )

    assert data.train_mask.shape== (203769, )
    assert data.train_mask.sum() > 0
    assert data.test_mask.shape== (203769, )
