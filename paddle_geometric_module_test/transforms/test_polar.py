...
from paddle_geometric.transforms import Polar


def test_polar():
    assert str(Polar()) == 'Polar(norm=True, max_value=None)'

    pos = paddle.to_tensor([[0.0, 0.0], [1.0, 0.0]])
    ...
    assert paddle.allclose(
        data.edge_attr,
        paddle.to_tensor([[1.0, 0.0], [1.0, PI]]),
        atol=1e-4,
    )
    ...
    assert paddle.allclose(
        data.edge_attr,
        paddle.to_tensor([[1.0, 1.0, 0.0], [1.0, 1.0, 0.5]]),
        atol=1e-4,
    )
