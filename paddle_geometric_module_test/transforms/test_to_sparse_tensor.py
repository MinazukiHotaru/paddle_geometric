...
@pytest.mark.parametrize('layout', [None, paddle.sparse_coo, paddle.sparse_csr])
def test_to_sparse_tensor_basic(layout):
    ...
    assert paddle.equal(data.edge_attr, edge_attr[perm])
    assert 'adj_t' in data

    if layout is None and paddle_geometric.typing.WITH_PADDLE_SPARSE:
        row, col, value = data.adj_t.coo()
        assert row.tolist() == [0, 1, 1, 2]
        assert col.tolist() == [1, 0, 2, 1]
        assert paddle.equal(value, edge_weight[perm])
    else:
        adj_t = data.adj_t
        assert adj_t.layout == layout or paddle.sparse_csr
        ...
        assert paddle.equal(adj_t.coalesce().values(), edge_weight[perm])
...
def test_to_sparse_tensor_and_keep_edge_index():
    ...
    assert len(data) == 5
    assert paddle.equal(data.edge_index, edge_index[:, perm])
    assert paddle.equal(data.edge_weight, edge_weight[perm])
    assert paddle.equal(data.edge_attr, edge_attr[perm])
