...
from paddle_geometric.testing import minPython, withPackage
from paddle_geometric.transforms import AddSelfLoops
...
def test_to_dataclass():
    ...
    assert fields['_target_'].default == (
        'paddle_geometric.transforms.add_self_loops.AddSelfLoops')

    cfg = AddSelfLoopsConfig()
    assert str(cfg) == ("AddSelfLoops(attr='edge_weight', fill_value=1.0, "
                        "_target_='paddle_geometric.transforms.add_self_loops."
                        "AddSelfLoops')")
