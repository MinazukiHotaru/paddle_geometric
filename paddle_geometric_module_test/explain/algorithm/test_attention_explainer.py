...
from paddle_geometric.explain import AttentionExplainer, Explainer
from paddle_geometric.explain.config import ExplanationType, MaskType
from paddle_geometric.nn import AttentiveFP, GATConv, GATv2Conv, TransformerConv


class AttentionGNN(paddle.nn.Layer):
    def __init__(self):
        super().__init__()
...
edge_attr = paddle.randn(shape=[edge_index.shape[1], 5])
batch = paddle.to_tensor([0, 0, 0, 1, 1, 2, 2, 2])


@pytest.mark.parametrize('index', [None, 2, paddle.arange(3)])
