...
from paddle_geometric.inspector import Inspector, Parameter, Signature
from paddle_geometric.nn import GATConv, SAGEConv
from paddle_geometric.typing import OptPairTensor
...
def test_inspector_sage_conv() -> None:
    ...
    kwargs = {'x_j': paddle.randn(shape=[[5]]), 'x_i': paddle.randn(shape=[[5]])}
    data = inspector.collect_param_data('message', kwargs)
    assert len(data) == 1
    assert paddle.allclose(data['x_j'], kwargs['x_j'])
...
def test_inspector_gat_conv() -> None:
    ...
    kwargs = {'x_j': paddle.randn(shape=[[5]]), 'alpha': paddle.randn(shape=[[5]])}
    data = inspector.collect_param_data('message', kwargs)
    assert len(data) == 2
    assert paddle.allclose(data['x_j'], kwargs['x_j'])
    assert paddle.allclose(data['alpha'], kwargs['alpha'])
