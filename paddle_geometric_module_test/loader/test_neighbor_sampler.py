...
from paddle_geometric.nn.conv import GATConv, SAGEConv
from paddle_geometric.testing import onlyOnline, withPackage
from paddle_geometric.typing import SparseTensor
from paddle_geometric.utils import erdos_renyi_graph
...
def test_neighbor_sampler_basic():
    ...
    for batch_size, n_id, adjs in loader:
        assert batch_size == 2
        assert all(np.isin(n_id, paddle.arange(10)).tolist())
        assert n_id.unique().shape[0] == n_id.shape[0]
        for (edge_index, e_id, size) in adjs:
            assert int(edge_index[0].max() + 1) <= size[0]
            assert int(edge_index[1].max() + 1) <= size[1]
            assert all(np.isin(e_id, paddle.arange(E)).tolist())
            assert e_id.unique().shape[0] == e_id.shape[0]
    ...
    for batch_size, n_id, adjs in loader:
        for (adj_t, e_id, size) in adjs:
            assert adj_t.shape[0] == size[1]
            assert adj_t.shape[1] == size[0]
