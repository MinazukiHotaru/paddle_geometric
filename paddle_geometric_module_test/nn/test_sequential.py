...
import paddle
import paddle.fx
from paddle.nn import Dropout, Linear, ReLU
...
def test_sequential_basic():
    ...
    batch = paddle.zeros(4, dtype=paddle.int64)
...
def test_sequential_jit():
    ...
    paddle.jit.to_static(model)(x, edge_index)
    ...
    if paddle_geometric.typing.WITH_PADDLE_SPARSE:
        ...
        paddle.jit.to_static(model)(x, adj_t)
...
def symbolic_trace(module):
    ...
    return paddle.fx.GraphModule(module, Tracer().trace(module))
...
def test_sequential_to_hetero():
    ...
    assert out_dict['paper'].shape== (100, 64)
    assert out_dict['author'].shape== (100, 64)
