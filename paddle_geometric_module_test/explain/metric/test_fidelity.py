...
from paddle_geometric.explain import (
    DummyExplainer,
    Explainer,
...
def test_fidelity(explanation_type):
    ...
    if explanation_type == 'phenomenon':
        target = paddle.randint(0, x.shape[1], (x.shape[0], ))

    explanation = explainer(x, edge_index, target=target,
                            index=paddle.arange(4))
...
def test_characterization_score():
    out = characterization_score(
        pos_fidelity=paddle.to_tensor([1.0, 0.6, 0.5, 1.0]),
        neg_fidelity=paddle.to_tensor([0.0, 0.2, 0.5, 1.0]),
        pos_weight=0.2,
        neg_weight=0.8,
    )
...
def test_fidelity_curve_auc():
    pos_fidelity = paddle.to_tensor([1.0, 1.0, 0.5, 1.0])
    neg_fidelity = paddle.to_tensor([0.0, 0.5, 0.5, 0.9])

    x = paddle.to_tensor([0, 1, 2, 3])
    out = round(float(fidelity_curve_auc(pos_fidelity, neg_fidelity, x)), 4)
    assert out == 8.5

    x = paddle.to_tensor([10, 11, 12, 13])
    out = round(float(fidelity_curve_auc(pos_fidelity, neg_fidelity, x)), 4)
    assert out == 8.5

    x = paddle.to_tensor([0, 1, 2, 5])
    out = round(float(fidelity_curve_auc(pos_fidelity, neg_fidelity, x)), 4)
    assert out == 19.5
