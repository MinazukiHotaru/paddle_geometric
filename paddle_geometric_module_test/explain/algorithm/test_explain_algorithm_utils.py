...
from paddle_geometric.nn import GCNConv, HeteroConv, SAGEConv, to_hetero


class HeteroModel(paddle.nn.Layer):
    def __init__(self):
        super().__init__()
...
def test_set_clear_mask(hetero_data):
    edge_mask_dict = {
        ('paper', 'to', 'paper'): paddle.ones(200),
        ('author', 'to', 'paper'): paddle.ones(100),
        ('paper', 'to', 'author'): paddle.ones(100),
    }
    ...
    for edge_type in hetero_data.edge_types:
        ...
        assert paddle.allclose(model.conv1.convs[edge_type]._edge_mask,
    ...
    for edge_type in hetero_data.edge_types:
        ...
        assert paddle.allclose(model.conv1[str_edge_type]._edge_mask,
                              edge_mask_dict[edge_type])
