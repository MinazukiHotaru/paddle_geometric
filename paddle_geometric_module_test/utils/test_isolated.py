...
def test_contains_isolated_nodes():
    ...
    if is_full_test():
        jit = paddle.jit.to_static(contains_isolated_nodes)
        assert not jit(edge_index)
        assert jit(edge_index, num_nodes=3)

    edge_index = paddle.to_tensor([[0, 1, 2, 0], [1, 0, 2, 0]])
...
def test_remove_isolated_nodes():
    ...
    if is_full_test():
        jit = paddle.jit.to_static(remove_isolated_nodes)
    ...
    edge_index = paddle.to_tensor([[0, 2, 1, 0, 2], [2, 0, 1, 0, 2]])
