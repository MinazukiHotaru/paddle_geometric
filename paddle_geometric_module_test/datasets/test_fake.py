...
def test_fake_dataset(num_graphs, edge_dim, task):
    ...
    if edge_dim == 0:
        ...
        assert len(data) == 5
        assert data.edge_weight.shape== (data.num_edges, )
        assert data.edge_weight.min() >= 0 and data.edge_weight.max() < 1
    else:
        assert len(data) == 5
        assert data.edge_attr.shape== (data.num_edges, edge_dim)
    ...
    assert data.global_features.shape== (3, )
...
def test_fake_hetero_dataset(num_graphs, edge_dim, task):
    ...
    for store in data.node_stores:
        ...
        if task == 'node' or (task == 'auto' and num_graphs == 1):
            if store._key == 'v0':
                ...
                assert store.y.shape== (store.num_nodes, )
    ...
    for store in data.edge_stores:
        if edge_dim == 0:
            ...
            assert len(data) == 5
            assert store.edge_weight.shape== (store.num_edges, )
            assert store.edge_weight.min() >= 0 and store.edge_weight.max() < 1
        else:
            assert len(data) == 5
            assert store.edge_attr.shape== (store.num_edges, edge_dim)
