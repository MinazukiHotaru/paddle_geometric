...
from paddle_geometric.nn import GraphConv, to_hetero
...
DTYPES = [
    pytest.param(paddle.int64, id='int64'),
    pytest.param(paddle.int32, id='int32'),
]
...
def is_subset(subedge_index, edge_index, src_idx, dst_idx):
    ...
    mask = paddle.from_numpy(np.isin(subidx.cpu().numpy(), idx.cpu().numpy()))
...
def test_homo_neighbor_loader_basic(
    ...
    if dtype != paddle.int64:
        return
    induced = SubgraphType.induced
    if subgraph_type == SubgraphType.induced and not WITH_PADDLE_SPARSE:
        return
    if dtype != paddle.int64 and (not WITH_PYG_LIB or subgraph_type == induced):
        return
    ...
    data.x = paddle.arange(100, place=device)
    data.edge_index = get_random_edge_index(100, 100, 500, dtype, device)
    data.edge_attr = paddle.arange(500, place=device)
    ...
    for i, batch in enumerate(loader):
        ...
        assert batch.n_id.shape== (batch.num_nodes, )
        ...
        assert paddle.equal(
            ...
            paddle.arange(i * batch.batch_size, (i + 1) * batch.batch_size,
                         place=device),
        )

        if subgraph_type != SubgraphType.bidirectional:
            assert batch.e_id.shape== (batch.num_edges, )
            assert batch.edge_attr.min() >= 0
            assert batch.edge_attr.max() < 500

            assert is_subset(
                batch.edge_index.to(paddle.int64),
                data.edge_index.to(paddle.int64),
                batch.x,
                batch.x,
            )
...
def test_hetero_neighbor_loader_basic(subgraph_type, dtype):
    ...
    data['paper'].x = paddle.arange(100)
    data['author'].x = paddle.arange(100, 300)

    edge_index = get_random_edge_index(100, 100, 500, dtype)
    data['paper', 'paper'].edge_index = edge_index
    data['paper', 'paper'].edge_attr = paddle.arange(500)
    edge_index = get_random_edge_index(100, 200, 1000, dtype)
    data['paper', 'author'].edge_index = edge_index
    data['paper', 'author'].edge_attr = paddle.arange(500, 1500)
    edge_index = get_random_edge_index(200, 100, 1000, dtype)
    data['author', 'paper'].edge_index = edge_index
    data['author', 'paper'].edge_attr = paddle.arange(1500, 2500)

    r1, c1 = data['paper', 'paper'].edge_index
    r2, c2 = data['paper', 'author'].edge_index + paddle.to_tensor([[0], [100]])
    r3, c3 = data['author', 'paper'].edge_index + paddle.to_tensor([[100], [0]])
    ...
    for batch in loader:
        ...
        assert batch['paper'].n_id.shape== (batch['paper'].num_nodes, )
        ...
        assert batch['author'].n_id.shape== (batch['author'].num_nodes, )
        ...
        if subgraph_type != SubgraphType.bidirectional:
            assert batch['paper', 'paper'].e_id.shape== (row.numel(), )
            value = batch['paper', 'paper'].edge_attr
            assert value.min() >= 0 and value.max() < 500

            assert is_subset(
                batch['paper', 'paper'].edge_index.to(paddle.int64),
                data['paper', 'paper'].edge_index.to(paddle.int64),
                batch['paper'].x,
                batch['paper'].x,
            )
        ...
        if subgraph_type != SubgraphType.bidirectional:
            assert batch['paper', 'author'].e_id.shape== (row.numel(), )
            value = batch['paper', 'author'].edge_attr
            assert value.min() >= 500 and value.max() < 1500

            assert is_subset(
                batch['paper', 'author'].edge_index.to(paddle.int64),
                data['paper', 'author'].edge_index.to(paddle.int64),
                batch['paper'].x,
                batch['author'].x - 100,
            )
        elif subgraph_type != SubgraphType.directional:
        ...
        if subgraph_type != SubgraphType.bidirectional:
            assert batch['author', 'paper'].e_id.shape== (row.numel(), )
            value = batch['author', 'paper'].edge_attr
            assert value.min() >= 1500 and value.max() < 2500

            assert is_subset(
                batch['author', 'paper'].edge_index.to(paddle.int64),
                data['author', 'paper'].edge_index.to(paddle.int64),
                batch['author'].x - 100,
                batch['paper'].x,
            )
        elif subgraph_type != SubgraphType.directional:
...
def test_temporal_hetero_neighbor_loader_on_karate(get_dataset):
    ...
    hetero_data = HeteroData()
    hetero_data['v'].x = data.x
    hetero_data['v'].time = paddle.arange(data.num_nodes, 0, -1)
    hetero_data['v', 'v'].edge_index = data.edge_index
...
def test_custom_neighbor_loader():
    ...
    edge_index = edge_index[:, paddle.randperm(edge_index.shape[1])]
    ...
    for batch1, batch2 in zip(loader1, loader2):
        ...
        assert paddle.allclose(batch1.x.sort()[0], batch2.x.sort()[0])
        assert paddle.allclose(batch1.y.sort()[0], batch2.y.sort()[0])
...
def test_custom_hetero_neighbor_loader():
    ...
    adj = to_paddle_csr_tensor(edge_index, size=(100, 200))
    ...
    adj = to_paddle_csr_tensor(edge_index.flip([0]), size=(100, 200))
...
def test_temporal_custom_neighbor_loader_on_karate(get_dataset):
    dataset = get_dataset(name='karate')
    data = dataset[0]
    data.time = paddle.arange(data.num_nodes, 0, -1)
    ...
    for batch1, batch2 in zip(loader1, loader2):
        assert paddle.equal(batch1['v'].time, batch2['v'].time)


@withPackage('pyg_lib', 'paddle_sparse')
def test_pyg_lib_and_paddle_sparse_homo_equality():
    edge_index = get_random_edge_index(20, 20, 100)
    adj = to_paddle_csr_tensor(edge_index.flip([0]), size=(20, 20))
    colptr, row = adj.crow_indices(), adj.col_indices()

    seed = paddle.arange(10)

    sample = paddle.ops.pyg.neighbor_sample
    out1 = sample(colptr, row, seed, [-1, -1], None, None, None, None, True)
    sample = paddle.ops.paddle_sparse.neighbor_sample
    out2 = sample(colptr, row, seed, [-1, -1], False, True)

    row1, col1, node_id1, edge_id1 = out1[:4]
    node_id2, row2, col2, edge_id2 = out2
    assert paddle.equal(node_id1, node_id2)
    assert paddle.equal(row1, row2)
    assert paddle.equal(col1, col2)
    assert paddle.equal(edge_id1, edge_id2)


@withPackage('pyg_lib', 'paddle_sparse')
def test_pyg_lib_and_paddle_sparse_hetero_equality():
    edge_index = get_random_edge_index(20, 10, 50)
    adj = to_paddle_csr_tensor(edge_index.flip([0]), size=(10, 20))
    colptr1, row1 = adj.crow_indices(), adj.col_indices()

    edge_index = get_random_edge_index(10, 20, 50)
    adj = to_paddle_csr_tensor(edge_index.flip([0]), size=(20, 10))
    ...
    seed_dict = {'paper': paddle.arange(1)}
    num_neighbors_dict = {
        'paper__to__author': [-1, -1],
        'author__to__paper': [-1, -1],
    }

    sample = paddle.ops.pyg.hetero_neighbor_sample
    ...
    sample = paddle.ops.paddle_sparse.hetero_neighbor_sample
    ...
    for key in node_id1_dict.keys():
        assert paddle.equal(node_id1_dict[key], node_id2_dict[key])
    assert len(row1_dict) == len(row2_dict)
    for key in row1_dict.keys():
        assert paddle.equal(row1_dict[key], row2_dict[key])
    assert len(col1_dict) == len(col2_dict)
    for key in col1_dict.keys():
        assert paddle.equal(col1_dict[key], col2_dict[key])
    assert len(edge_id1_dict) == len(edge_id2_dict)
    for key in edge_id1_dict.keys():
        assert paddle.equal(edge_id1_dict[key], edge_id2_dict[key])
...
def test_memmap_neighbor_loader(tmp_path):
    ...
    x[:] = np.random.randn(shape=[100, 32])
    ...
    assert isinstance(batch.x, paddle.to_tensor)
    assert batch.x.shape== (batch.num_nodes, 32)
...
def test_weighted_homo_neighbor_loader():
    ...
    edge_weight = paddle.to_tensor([0.0, 1.0, 0.0, 1.0])

    data = Data(num_nodes=5, edge_index=edge_index, edge_weight=edge_weight)

    loader = NeighborLoader(
        data,
        input_nodes=paddle.to_tensor([2]),
        num_neighbors=[1] * 2,
        batch_size=1,
...
def test_weighted_hetero_neighbor_loader():
    ...
    loader = NeighborLoader(
        data,
        input_nodes=('paper', paddle.to_tensor([2])),
        num_neighbors=[1] * 2,
        batch_size=1,
        weight_attr='edge_weight',
    )
...
def test_edge_level_temporal_homo_neighbor_loader():
    ...
    edge_time = paddle.arange(edge_index.shape[1])
    ...
    loader = NeighborLoader(
        data,
        num_neighbors=[-1, -1],
        input_time=paddle.to_tensor([4, 4, 4, 4, 4]),
        time_attr='edge_time',
        batch_size=1,
    )
...
def test_neighbor_loader_with_tensor_frame(device):
    data = Data()
    data.tf = get_random_tensor_frame(num_rows=100, place=device)
    data.edge_index = get_random_edge_index(100, 100, 500, place=device)
    data.edge_attr = get_random_tensor_frame(500, place=device)
    data.global_tf = get_random_tensor_frame(num_rows=1, place=device)
...
def test_neighbor_loader_input_id():
    ...
    row = paddle.randint(0, data['a'].num_nodes, (40, ))
    col = paddle.randint(0, data['b'].num_nodes, (40, ))
    data['a', 'b'].edge_index = paddle.stack([row, col], dim=0)
    data['b', 'a'].edge_index = paddle.stack([col, row], dim=0)

    mask = paddle.ones(data['a'].num_nodes, dtype=paddle.bool)
    mask[0] = False
...
def test_temporal_neighbor_loader_single_link():
    data = HeteroData()
    data['a'].x = paddle.arange(10)
    data['b'].x = paddle.arange(10)
    data['c'].x = paddle.arange(10)

    data['b'].time = paddle.arange(0, 10)
    data['c'].time = paddle.arange(1, 11)

    data['a', 'b'].edge_index = paddle.arange(10).view(1, -1).repeat(2, 1)
    data['b', 'a'].edge_index = paddle.arange(10).view(1, -1).repeat(2, 1)
    data['a', 'c'].edge_index = paddle.arange(10).view(1, -1).repeat(2, 1)
    data['c', 'a'].edge_index = paddle.arange(10).view(1, -1).repeat(2, 1)
