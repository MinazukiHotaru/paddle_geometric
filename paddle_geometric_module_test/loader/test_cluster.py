...
from paddle_geometric.loader import ClusterData, ClusterLoader
from paddle_geometric.testing import onlyFullTest, onlyOnline, withMETIS
...
def test_cluster_gcn():
    ...
    partition = cluster_data._partition(
        edge_index, cluster=paddle.to_tensor([0, 1, 0, 1, 0, 1]))
    ...
    assert paddle.equal(
        ...
        paddle.arange(data.num_nodes),
    )
    assert paddle.equal(
        cluster_data.partition.edge_perm.sort()[0],
        paddle.arange(data.num_edges),
    )
    ...
    assert out.n_id.shape== (3, )
    assert paddle.equal(out.x, expected.x)
    tmp = sort_edge_index(expected.edge_index, expected.edge_attr)
    assert paddle.equal(out.edge_index, tmp[0])
    assert paddle.equal(out.edge_attr, tmp[1])
...
def test_cluster_gcn_correctness(get_dataset, sparse_format):
    ...
    data.n_id = paddle.arange(data.num_nodes)
    ...
    for batch1 in loader:
        ...
        assert paddle.equal(batch1.x, batch2.x)
        assert paddle.equal(
            batch1.edge_index,
            sort_edge_index(
                batch2.edge_index,
