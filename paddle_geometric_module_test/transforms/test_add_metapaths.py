...
from paddle_geometric.transforms import AddMetaPaths, AddRandomMetaPaths
from paddle_geometric.utils import coalesce


def generate_data() -> HeteroData:
    data = HeteroData()
    data['p'].x = paddle.ones(5)
    data['a'].x = paddle.ones(6)
    data['c'].x = paddle.ones(3)
    data['p', 'p'].edge_index = paddle.to_tensor([[0, 1, 2, 3], [1, 2, 4, 2]])
    data['p', 'a'].edge_index = paddle.to_tensor([[0, 1, 2, 3, 4], [2, 2, 5, 2, 5]])
    data['a', 'p'].edge_index = data['p', 'a'].edge_index.flip([0])
    data['c', 'p'].edge_index = paddle.to_tensor([[0, 0, 1, 2, 2], [0, 1, 2, 3, 4]])
...
def test_add_metapaths() -> None:
    ...
    assert meta1['metapath_0'].edge_index.shape== (2, 9)
    assert meta2['metapath_0'].edge_index.shape== (2, 9)
    assert meta3['metapath_0'].edge_index.shape== (2, 9)
    assert meta4['metapath_0'].edge_index.shape== (2, 9)
    ...
    assert meta['metapath_0'].edge_index.shape== (2, 4)
    assert meta['metapath_1'].edge_index.shape== (2, 4)
...
def test_add_metapaths_max_sample() -> None:
    ...
    meta = transform(data)
    assert meta['metapath_0'].edge_index.shape[1] < 9
...
def test_add_random_metapaths() -> None:
    ...
    assert meta1['metapath_0'].edge_index.shape== (2, 5)
    assert meta2['metapath_0'].edge_index.shape== (2, 5)
    assert meta3['metapath_0'].edge_index.shape== (2, 5)
    assert meta4['metapath_0'].edge_index.shape== (2, 5)
    assert meta5['metapath_0'].edge_index.shape== (2, 4)
    assert meta6['metapath_0'].edge_index.shape== (2, 7)
    ...
    meta1 = transform(data)
    new_edge_types = [('a', 'metapath_0', 'c'), ('a', 'metapath_1', 'a')]
    assert meta1['metapath_0'].edge_index.shape== (2, 2)
    assert meta1['metapath_1'].edge_index.shape== (2, 2)
    ...
    meta2 = transform(data)
    new_edge_types = [('a', 'metapath_0', 'c'), ('a', 'metapath_1', 'a')]
    assert meta2['metapath_0'].edge_index.shape== (2, 2)
    assert meta2['metapath_1'].edge_index.shape== (2, 3)
