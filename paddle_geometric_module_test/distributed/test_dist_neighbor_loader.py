...
def dist_neighbor_loader_hetero(
    ...
    for batch in loader:
        ...
        for node_type in batch.node_types:
            assert paddle.equal(batch[node_type].x, batch.x_dict[node_type])
            assert batch.x_dict[node_type].shape[0] >= 0
            assert batch[node_type].n_id.shape[0] == batch[node_type].num_nodes
        ...
        for edge_type in batch.edge_types:
            ...
            if num_edges > 0:  # Test edge mapping:
                assert batch[edge_type].edge_attr.shape[0] == num_edges
                src, _, dst = edge_type
                edge_index = part_data[1]._edge_index[(edge_type, "coo")]
                global_edge_index1 = paddle.stack([
                ...
                if e_id.numel() > 0 and e_id.max() >= edge_index.shape[1]:
                    warnings.warn("Known test flake")
                else:
                    global_edge_index2 = edge_index[:, e_id]
                    if not paddle.equal(global_edge_index1, global_edge_index2):
                        warnings.warn("Known test flake")
