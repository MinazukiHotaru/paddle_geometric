...
from paddle_geometric.transforms import IndexToMask, MaskToIndex
...
def test_index_to_mask():
    ...
    edge_index = paddle.to_tensor([[0, 1, 1, 2, 2, 3, 3, 4],
                               [1, 0, 2, 1, 3, 2, 4, 3]])
    train_index = paddle.arange(0, 3)
    test_index = paddle.arange(3, 5)
...
def test_mask_to_index():
    ...
    train_mask = paddle.to_tensor([True, True, True, False, False])
    test_mask = paddle.to_tensor([False, False, False, True, True])
...
def test_hetero_index_to_mask():
    data = HeteroData()
    data['u'].train_index = paddle.arange(0, 3)
    data['u'].test_index = paddle.arange(3, 5)
    data['u'].num_nodes = 5

    data['v'].train_index = paddle.arange(0, 3)
    data['v'].test_index = paddle.arange(3, 5)
    data['v'].num_nodes = 5
...
def test_hetero_mask_to_index():
    data = HeteroData()
    data['u'].train_mask = paddle.to_tensor([True, True, True, False, False])
    data['u'].test_mask = paddle.to_tensor([False, False, False, True, True])

    data['v'].train_mask = paddle.to_tensor([True, True, True, False, False])
    data['v'].test_mask = paddle.to_tensor([False, False, False, True, True])
